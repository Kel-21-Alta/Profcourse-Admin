/** @format */

import { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import Logo from "../../assets/profcourseLogo.png";
export default function Sidebar(props) {
  const [active] = useState(props?.active);
  console.log(active);
  console.log(props.active);

  useEffect(() => {}, [active]);
  return (
    <div className="col-auto col-md-3 col-xl-2 px-sm-2 px-0 bg-secondary">
      <div className="d-flex flex-column align-items-center align-items-sm-start px-3 pt-2 min-vh-100">
        <a href="/" className="mx-auto justify-content-center pe-2 my-3">
          <img src={Logo} alt="img-fluid" />
        </a>
        <ul
          className="nav nav-pills flex-column mb-sm-auto mb-0 align-items-center align-items-sm-start mt-3"
          id="menu">
          <li className="nav-item">
            <Link to="/dashboard" className="nav-link px-0 align-middle">
              <i className="fs-4 bi-speedometer2">
                <svg
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M2.83201 19C2.28142 17.7384 1.99813 16.3765 2.00001 15C2.00001 9.477 6.47701 5 12 5C17.523 5 22 9.477 22 15C22.0019 16.3765 21.7186 17.7384 21.168 19H2.83201ZM19.748 17C20.0515 15.8183 20.0807 14.5828 19.8334 13.388C19.586 12.1933 19.0687 11.0709 18.321 10.1068C17.5732 9.14276 16.6148 8.36248 15.5192 7.82569C14.4235 7.2889 13.2196 7.00984 11.9995 7.00984C10.7794 7.00984 9.5755 7.2889 8.47985 7.82569C7.38419 8.36248 6.42582 9.14276 5.67807 10.1068C4.93031 11.0709 4.41297 12.1933 4.16563 13.388C3.91828 14.5828 3.94749 15.8183 4.25101 17H19.748ZM12 10C11.7348 10 11.4804 9.89464 11.2929 9.70711C11.1054 9.51957 11 9.26522 11 9C11 8.73478 11.1054 8.48043 11.2929 8.29289C11.4804 8.10536 11.7348 8 12 8C12.2652 8 12.5196 8.10536 12.7071 8.29289C12.8947 8.48043 13 8.73478 13 9C13 9.26522 12.8947 9.51957 12.7071 9.70711C12.5196 9.89464 12.2652 10 12 10ZM7.00001 14C6.73479 14 6.48044 13.8946 6.2929 13.7071C6.10537 13.5196 6.00001 13.2652 6.00001 13C6.00001 12.7348 6.10537 12.4804 6.2929 12.2929C6.48044 12.1054 6.73479 12 7.00001 12C7.26523 12 7.51958 12.1054 7.70712 12.2929C7.89465 12.4804 8.00001 12.7348 8.00001 13C8.00001 13.2652 7.89465 13.5196 7.70712 13.7071C7.51958 13.8946 7.26523 14 7.00001 14ZM17 14C16.7348 14 16.4804 13.8946 16.2929 13.7071C16.1054 13.5196 16 13.2652 16 13C16 12.7348 16.1054 12.4804 16.2929 12.2929C16.4804 12.1054 16.7348 12 17 12C17.2652 12 17.5196 12.1054 17.7071 12.2929C17.8947 12.4804 18 12.7348 18 13C18 13.2652 17.8947 13.5196 17.7071 13.7071C17.5196 13.8946 17.2652 14 17 14ZM12 11C12.2652 11 12.5196 11.1054 12.7071 11.2929C12.8947 11.4804 13 11.7348 13 12V15C13 15.2652 12.8947 15.5196 12.7071 15.7071C12.5196 15.8946 12.2652 16 12 16C11.7348 16 11.4804 15.8946 11.2929 15.7071C11.1054 15.5196 11 15.2652 11 15V12C11 11.7348 11.1054 11.4804 11.2929 11.2929C11.4804 11.1054 11.7348 11 12 11Z"
                    fill={active === 1 ? "#3252df" : "#000000"}
                  />
                </svg>
              </i>{" "}
              <span
                className={
                  active === 1
                    ? `ms-1 d-none d-sm-inline text-thirtiery fw-bold`
                    : `ms-1 d-none d-sm-inline`
                }>
                Dashboard
              </span>{" "}
            </Link>
          </li>
          <li>
            <Link to="/kursus" className="nav-link px-0 align-middle">
              <i className="fs-4 bi-speedometer2">
                <svg
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M17.25 2C17.6111 2 17.9687 2.07113 18.3024 2.20933C18.636 2.34753 18.9392 2.5501 19.1945 2.80546C19.4499 3.06082 19.6525 3.36398 19.7907 3.69762C19.9289 4.03127 20 4.38886 20 4.75V19.25C20 19.6111 19.9289 19.9687 19.7907 20.3024C19.6525 20.636 19.4499 20.9392 19.1945 21.1945C18.9392 21.4499 18.636 21.6525 18.3024 21.7907C17.9687 21.9289 17.6111 22 17.25 22H6.75C6.38878 22 6.0311 21.9288 5.69739 21.7906C5.36368 21.6523 5.06048 21.4497 4.8051 21.1942C4.54973 20.9387 4.34719 20.6354 4.20904 20.3017C4.0709 19.9679 3.99987 19.6102 4 19.249V4.75C4 3.49 4.846 2.43 6 2.103V3.75C5.696 3.978 5.5 4.34 5.5 4.75V19.248C5.5 19.938 6.06 20.498 6.75 20.498H17.25C17.94 20.498 18.5 19.938 18.5 19.248V4.75C18.5 4.06 17.94 3.5 17.25 3.5H15V2H17.25ZM14 2V10.139C14 10.886 13.2 11.166 12.71 10.903L12.628 10.851L10.502 9.566L8.424 10.817C7.924 11.177 7.094 10.957 7.007 10.259L7 10.14V2H14ZM12.5 3.5H8.5V9.023L10.073 8.074C10.1983 8.00738 10.3373 7.97055 10.4792 7.96639C10.6211 7.96223 10.762 7.99084 10.891 8.05L12.501 9.024V3.5H12.5Z"
                    fill={active === 2 ? "#3252df" : "#000000"}
                  />
                </svg>
              </i>{" "}
              <span
                className={
                  active === 2
                    ? `ms-1 d-none d-sm-inline text-thirtiery fw-bold`
                    : `ms-1 d-none d-sm-inline`
                }>
                Kursus
              </span>{" "}
            </Link>
          </li>
          <li>
            <Link to="/pengguna" className="nav-link px-0 align-middle">
              <i className="fs-4 bi-table">
                <svg
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M12 2C11.0111 2 10.0444 2.29324 9.22215 2.84265C8.3999 3.39206 7.75904 4.17295 7.3806 5.08658C7.00216 6.00021 6.90315 7.00555 7.09607 7.97545C7.289 8.94536 7.7652 9.83627 8.46447 10.5355C9.16373 11.2348 10.0546 11.711 11.0245 11.9039C11.9945 12.0969 12.9998 11.9978 13.9134 11.6194C14.827 11.241 15.6079 10.6001 16.1573 9.77785C16.7068 8.95561 17 7.98891 17 7C17 5.67392 16.4732 4.40215 15.5355 3.46447C14.5979 2.52678 13.3261 2 12 2ZM12 10C11.4067 10 10.8266 9.82405 10.3333 9.49441C9.83994 9.16476 9.45542 8.69623 9.22836 8.14805C9.0013 7.59987 8.94189 6.99667 9.05764 6.41473C9.1734 5.83279 9.45912 5.29824 9.87868 4.87868C10.2982 4.45912 10.8328 4.1734 11.4147 4.05764C11.9967 3.94189 12.5999 4.0013 13.1481 4.22836C13.6962 4.45542 14.1648 4.83994 14.4944 5.33329C14.8241 5.82664 15 6.40666 15 7C15 7.79565 14.6839 8.55871 14.1213 9.12132C13.5587 9.68393 12.7956 10 12 10ZM21 21V20C21 18.1435 20.2625 16.363 18.9497 15.0503C17.637 13.7375 15.8565 13 14 13H10C8.14348 13 6.36301 13.7375 5.05025 15.0503C3.7375 16.363 3 18.1435 3 20V21H5V20C5 18.6739 5.52678 17.4021 6.46447 16.4645C7.40215 15.5268 8.67392 15 10 15H14C15.3261 15 16.5979 15.5268 17.5355 16.4645C18.4732 17.4021 19 18.6739 19 20V21H21Z"
                    fill={active === 3 ? "#3252df" : "#000000"}
                  />
                </svg>
              </i>{" "}
              <span
                className={
                  active === 3
                    ? `ms-1 d-none d-sm-inline text-thirtiery fw-bold`
                    : `ms-1 d-none d-sm-inline`
                }>
                Pengguna
              </span>
            </Link>
          </li>
          <li>
            <Link
              to="/permintaan"
              data-bs-toggle="collapse"
              className="nav-link px-0 align-middle ">
              <i className="fs-4 bi-bootstrap">
                <svg
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M20 20H4C3.46957 20 2.96086 19.7893 2.58579 19.4142C2.21071 19.0391 2 18.5304 2 18V5.913C2.02243 5.39779 2.24301 4.91116 2.61568 4.5547C2.98835 4.19824 3.4843 3.99951 4 4H20C20.5304 4 21.0391 4.21071 21.4142 4.58579C21.7893 4.96086 22 5.46957 22 6V18C22 18.5304 21.7893 19.0391 21.4142 19.4142C21.0391 19.7893 20.5304 20 20 20ZM4 7.868V18H20V7.868L12 13.2L4 7.868ZM4.8 6L12 10.8L19.2 6H4.8Z"
                    fill={active === 4 ? "#3252df" : "#000000"}
                  />
                </svg>
              </i>{" "}
              <span
                className={
                  active === 4
                    ? `ms-1 d-none d-sm-inline text-thirtiery fw-bold`
                    : `ms-1 d-none d-sm-inline`
                }>
                Permintaan
              </span>
            </Link>
          </li>
        </ul>
      </div>
    </div>
  );
}
